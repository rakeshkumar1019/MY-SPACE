		        	...
		        	connection.query(dash,function(err,users,fields){
		        		if(err) throw err
		        		    ...
		        		    connection.query(imgsql,function(err,imgsrc,fields){
		        		    	if(err) throw err
		        		    		var todaydate='select * from task where completedDate= "'+today +'" AND email="'+email+'" '
		        		    	     connection.query(todaydate,function(err,todaydate,fields){
		        		    	     	if(err) throw err
		 
		        		    	     	res.render('dashboard',{user:users[0],imagesrc:imgsrc[0],todaydata:todaydate})	
		        		    	     })
...
`
   ...
   sql='insert into task values("'+id+'","'+createdDate+'","'+title+'","'+content+'","'+completedDate+'","'+email+'","'+isDone+'","'+time+'")'
   connection.query(sql,function(err){
   	if(err) throw err
